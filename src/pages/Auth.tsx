  ...
  const formContent = <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
        ...
        field
              ...
              <FormControl>
                <div className="relative">
                  <Input {...field} type={showPassword ? "text" : "password"} disabled={isSubmitting} className={`${inputClassName} pr-10`} />
                  <button type="button" onClick={() => setShowPassword(prev => !prev)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors" aria-label={showPassword ? "Ocultar senha" : "Mostrar senha"} disabled={isSubmitting}>
        ...
        <div className="flex flex-row flex-wrap items-center justify-center gap-x-4 gap-y-2 pt-2">
          <button type="button" onClick={handleForgotPassword} disabled={isSubmitting || resettingPassword} className="text-sm text-gray-500 hover:text-gray-700 transition-colors flex items-center">
            {resettingPassword && <Loader2 className="mr-2 h-3 w-3 animate-spin" />}
            Esqueci a senha
          </button>
          <span className="text-gray-300">·</span>
          <a href="https://eonhub.com.br/sign" target="_blank" rel="noopener noreferrer" className="text-sm text-gray-500 hover:text-gray-700 transition-colors">
            Criar nova conta
          </a>
          <span className="text-gray-300">·</span>
          <button type="button" onClick={() => navigate("/install")} className="text-sm text-gray-500 hover:text-gray-700 transition-colors">
            Instale o App
          </button>
        </div>
      </form>
  ...
  return <div className="light" style={{
    colorScheme: 'light'
      ...
      backgroundColor: '#ffffff'
        ...
        <div className="relative flex-shrink-0 pt-[env(safe-area-inset-top)] px-6 pb-28" style={{
        ...
        <div className="flex-1 rounded-t-3xl -mt-6 px-6 py-5 relative z-30 flex flex-col" style={{
        backgroundColor: '#ffffff'
      }}>
          {/* Header */}
