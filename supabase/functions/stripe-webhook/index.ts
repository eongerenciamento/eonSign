    ...
    try {
      switch (event.type) {
        case "checkout.session.completed": {
          ...
          if (productType === "certificate" && session.mode === "payment") {
            const certName = session.metadata?.name;
            const certCpf = session.metadata?.cpf;
            const certEmail = session.metadata?.email;
            const certPhone = session.metadata?.phone;
            const certBirthDate = session.metadata?.birth_date;
            const certType = session.metadata?.type || "PF";
            const certCnpj = session.metadata?.cnpj || null;
            const certResponsibleName = session.metadata?.responsible_name || null;
            const certUserId = session.metadata?.user_id;

            logStep("Processing certificate payment", { certName, certEmail, certUserId });

            if (!certUserId) {
              throw new Error("User ID not found in metadata for certificate payment");
            }

            // Create certificate request record with paid status
            const { error: certError } = await supabaseClient
              ...
              .insert({
                ...
                email: certEmail,
                phone: certPhone,
                birth_date: certBirthDate,
                type: certType,
                cnpj: certCnpj,
                responsible_name: certResponsibleName,
                status: "paid",
                payment_status: "paid",
            ...
            if (certError) {
              logStep("Error creating certificate request", { error: certError.message });
              throw certError;
            }

            logStep("Certificate request created with paid status");

            // Send confirmation email
            try {
              await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/send-certificate-purchase-email`, {
                method: 'POST',
                headers: {
              ...
              logStep("Certificate purchase email sent");
            } catch (emailError) {
              logStep("Error sending certificate purchase email", { error: String(emailError) });
